...
class Game:
    ...
    def level(self, target_lvl, biome: str):
        ...
        self.cursor = Cursor(
            self.settings,
            (self.sprite_groups.all_sprites, self.sprite_groups.cursor_sprite),
    ...
    def update(self, dt: float) -> (int, int):
        ...
        """
        self.sprite_groups.walls.update(self.player.pos, self.level, self.data.biomes)
        self.sprite_groups.stops_bullets.update(
            self.player.pos, self.level, self.data.biomes
        )
        for mob in self.sprite_groups.mobs:
        ...
        self.sprite_groups.bullets.update(dt)
        self.sprite_groups.graves.update()
        self.sprite_groups.items.update()
        self.sprite_groups.player_sprite.update(
            self.cursor.pos,
        ...
        hits = pg.sprite.spritecollide(
            self.player, self.sprite_groups.items, False, collide_hit_rect
        )
        for hit in hits:
        ...
        # mobs hitting player
        hits = pg.sprite.spritecollide(
            self.player, self.sprite_groups.mobs, False, collide_hit_rect
        )
        for hit in hits:
        ...
        hits = pg.sprite.groupcollide(
            self.sprite_groups.mobs, self.sprite_groups.bullets, False, True
        )
        for hit in hits:
            hit.hp -= (
